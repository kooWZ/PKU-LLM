...
from utils.image_augmentations import *
from utils.mask_utils import *
from utils.minigpt_utils import (
    initialize_model,
    load_minigpt4,
    model_inference,
    query_minigpt,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="PL",
        type=str,
        help="Horizontal Flip(HF),Vertical Flip(VF),Random Rotation(RR),Crop and Resize(CR),Random Mask(RM),Random Solarization(RS),Random Grayscale(GR),Gaussian Blur(BL), Colorjitter(CJ), Random Posterization(RP) Policy(PL)",
    )
    parser.add_argument(
        "--serial_num",
        type=str,
    ...
    parser.add_argument(
        "--threshold", default=0.025, type=str, help="Threshold of divergence"
    )
    ...
    for i in range(number):
        tmp_method = img_aug_dict[args.mutator]
